...
const coverUpload = multer({
    ...
    fileFilter: (req, file, cb) => {
        const allowedMimes = ['image/jpeg', 'image/jpg', 'image/png', 'image/x-png', 'image/webp', 'application/octet-stream'];
 ...
 *   index 0 (ch1) → always free for everyone
 *   novel author  → always free for all chapters
 *   subscriber    → all chapters
 *   anyone else   → ch1 only
 */
const canReadChapter = (chapterIndex, userId, novelAuthorId, isAuthorOrSubscribed) => {
    if (chapterIndex === 0) return true;
    if (userId && novelAuthorId.toString() === userId.toString()) return true;
    if (isAuthorOrSubscribed) return true;
    return false;
};
...
router.get('/my', protect, async (req, res) => {
    try {
        const novels = await Novel.find({ author: req.user._id }).sort({ updatedAt: -1 });
...
router.get('/:id', optionalProtect, async (req, res) => {
    try {
        ...
        const authorId = novel.author._id.toString();
        ...
        formattedNovel.isOwned = isAuthor; // tells Flutter author badge
...
router.post('/', protect, async (req, res) => {
    try {
        ...
        const novel = await Novel.create({
            ...
            genre: genre || 'Other',
            tags: tags || [],
            freeChapterCount: freeChapterCount ?? 1,
...
router.delete('/:id/chapters/:chapterId', protect, async (req, res) => {
    try {
        ...
        novel.chapters = novel.chapters.filter(ch => ch._id.toString() !== req.params.chapterId);
        novel.totalChapters = novel.chapters.length;
        await novel.save();
