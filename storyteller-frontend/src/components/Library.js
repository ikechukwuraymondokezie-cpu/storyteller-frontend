...
export default function Library() {
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            // Delete from server (Assuming your API supports /api/books/:id)
            await Promise.all(
                selectedIds.map((id) =>
                    fetch(`${API_URL}/api/books/${id}`, { method: "DELETE" })
                )
            );
            ...
            alert("Some books could not be deleted.");
        }
    };
    ...
    return (
        <div className={`min-h-screen bg-bg px-6 py-8 ${isSelectMode ? "pb-32" : ""}`}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                    {books.map((book) => (
                        ...
                        >
                            ...
                            {/* RECTANGULAR COVER (2:3 Aspect Ratio) */}
                            ...
                            {/* Options Button (Hidden in Select Mode) */}
                            {!isSelectMode && (
                                <button
            ...
            {isSelectMode && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-zinc-900 border border-white/10 shadow-2xl rounded-2xl p-4 flex items-center justify-between z-[60] animate-in slide-in-from-bottom-10 duration-300">
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => { setIsSelectMode(false); setSelectedIds([]); }}
                            className="p-2 hover:bg-white/10 rounded-full text-zinc-400 transition"
                        >
                            <X size={20} />
                        </button>
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-2 rounded-xl font-bold transition-all ${selectedIds.length > 0 ? "bg-red-500 text-white shadow-lg shadow-red-500/20" : "bg-zinc-800 text-zinc-500 cursor-not-allowed"
                            }`}
                    >
                        <Trash2 size={18} />
                        Delete
            ...
            {activeBook && !isSelectMode && (
                ...
                >
                    <div
                        ...
                        className="w-full max-w-lg bg-zinc-900 rounded-t-3xl md:rounded-2xl pt-2 px-6 pb-8 md:pb-6"
                    >
                        ...
                        <div className="flex gap-4 mb-6 mt-2">
                            <img
                                ...
                                className="w-16 h-24 rounded-md object-cover shadow-lg"
                                onError={(e) => { e.target.src = defaultCover; }}
                            />
                        ...
                        <div className="space-y-3">
                            <button onClick={() => handleAction(activeBook._id, "download")} className="w-full flex items-center justify-center gap-3 bg-yellow-600 hover:bg-yellow-500 text-white py-3 px-4 rounded-xl font-semibold transition-colors">
                                <Download className="w-5 h-5" />
                                <span>Download PDF</span>
                            </button>
                            <button onClick={() => handleAction(activeBook._id, "tts")} className="w-full flex items-center justify-center gap-3 bg-white text-black hover:bg-zinc-200 py-3 px-4 rounded-xl font-semibold transition-colors">
                                <img src={f3logo} className="w-5 h-5" alt="f3" />
                                <span>Read with Funfiction</span>
                            </button>
                            <button onClick={() => alert("Coming soon")} className="w-full flex items-center justify-center gap-3 bg-zinc-800 hover:bg-zinc-700 text-white py-3 px-4 rounded-xl font-semibold transition-colors">
